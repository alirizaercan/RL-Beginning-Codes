...
class FrameStackPPOAgent:
    ...
    def update(self, episode):
        ...
        for t in range(len(self.memory)):
            ...
            # Backprop for critic
            d_value = 2 * (value - return_val) / len(self.memory)
            
            dW3_critic = np.dot(a2_critic.T, d_value)
            db3_critic = d_value
            
            d_a2_c = d_value * self.W3_critic.T
            d_a2_c = d_a2_c * relu_derivative(d_a2_c)
            
            dW2_critic = np.dot(a1_critic.T, d_a2_c)
            db2_critic = d_a2_c
            
            d_a1_c = np.dot(d_a2_c, self.W2_critic.T)
            d_a1_c = d_a1_c * relu_derivative(d_a1_c)
            
            dW1_critic = np.dot(state.T, d_a1_c)
            db1_critic = d_a1_c
    ...
    def train(self, env, episodes=500):
        ...
        for episode in range(episodes):
            ...
            # Episode'u tamamla, network'ü güncelle
            self.update(episode)
            ...
            if (episode + 1) % 100 == 0:
                ...
                print(f"Episode {episode+1}: Avg Reward (last 100)={avg_reward:.2f}, Current={episode_reward}")
...
if __name__ == "__main__":
    ...
    env = FrameStackingWrapper(env, num_frames=4, frame_size=84)
